...
var terrainToolClosure = function(ordinal){
  ...
  var newTool = function(scene, data){
    var that = this;
  ...
  newTool.prototype = new snorb.core.Tool();
  return newTool;
};

snorb.tools.terrainRaise = terrainToolClosure(1);
snorb.tools.terrainLower = terrainToolClosure(-1);
snorb.tools.terrainSmooth = terrainToolClosure(function(nearby, vertices){
  var altitudes = [];
  for(var r=0; r<nearby.length; r++){
    for(var i=0; i<nearby[r].length; i++){
