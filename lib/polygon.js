...
//downloaded from: http://freshplayer.blogspot.com/2014/04/intersection-of-two-2d-polygons-in.html

function samePolygon(polygon1, polygon2){
  if(polygon1.length !== polygon2.length){
    return false;
  };
  for(var i = 0; i<polygon1.length; i++){
    if(polygon1[i].x !== polygon2[i].x ||
       polygon1[i].y !== polygon2[i].y){
      return false;
    };
  };
  return true;
...
function intersectionPolygons(polygon1, polygon2) {
    ...
    if(samePolygon(polygon1, polygon2)){
      return polygon1;
    };
    ...
    while (!pointsEqual(currentPoint, startingPoint)) {
        ...
        if(foundIndices.indexOf(currentIndex) > -1){
            // Reached some kind of infinite loop?!
...
return {
  intersection: intersectionPolygons,
  pointInside: isPointInsidePolygon
};
})();
